...
fn org_string(id: ObjectId) -> String {
...
impl JobQueue for PostgresJobQueue {
    async fn enqueue(&self, job: NewJob) -> Result<JobId, JobQueueError> {
        let organization_id = org_string(job.organization_id);
        let payload = serde_json::to_value(&job.payload)
            .map_err(|error| JobQueueError::Serialization(error.to_string()))?;
        let inserted: Option<i64> = sqlx::query_scalar(
            "INSERT INTO jobs (organization_id, job_type, payload, status, attempts, max_retries, next_attempt_at_ms, deduplication_key, created_at_ms, updated_at_ms) VALUES ($1::uuid, $2, $3, 'pending', 0, $4, $5, $6, $7, $7) ON CONFLICT DO NOTHING RETURNING id",
        )
        ...
        let id: i64 = sqlx::query_scalar(
            "SELECT id FROM jobs WHERE deduplication_key = $1 ORDER BY id DESC LIMIT 1",
        )
        .bind(key)
        .fetch_one(&self.pool)
        .await
    ...
    async fn claim(
        ...
        let rows = sqlx::query(
            "SELECT id, organization_id::text AS organization_id, job_type, payload, attempts, max_retries FROM jobs WHERE status = 'pending' AND next_attempt_at_ms <= $1 ORDER BY next_attempt_at_ms, id FOR UPDATE SKIP LOCKED LIMIT $2",
        )
        .bind(now_ms)
        .bind(limit as i64)
        .fetch_all(&mut *tx)
        ...
        for row in rows {
            ...
            let organization: String = row
                ...
                .map_err(|e| JobQueueError::Persistence(e.to_string()))?;
            let organization = uuid::Uuid::parse_str(&organization)
            ...
            sqlx::query(
                "UPDATE jobs SET status = 'running', claimed_by = $1, lease_token = $2, lease_expires_at_ms = $3, attempts = attempts + 1, updated_at_ms = $4 WHERE id = $5",
            )
            .bind(worker_id)
            .bind(&lease_token)
            ...
            claimed.push(ClaimedJob {
                id: JobId(id),
                organization_id: ObjectId(*organization.as_bytes()),
                job_type,
                payload,
    ...
    async fn complete(&self, job_id: JobId, lease_token: &str) -> Result<(), JobQueueError> {
        let result = sqlx::query(
            "UPDATE jobs SET status = 'completed', claimed_by = NULL, lease_token = NULL, lease_expires_at_ms = NULL, updated_at_ms = $1 WHERE id = $2 AND status = 'running' AND lease_token = $3",
        )
        .bind(timestamp_ms(Timestamp::now()))
        .bind(job_id.0)
        .bind(lease_token)
    ...
    async fn fail(
        ...
        let row: Option<(i32, i32)> = sqlx::query_as(
            "SELECT attempts, max_retries FROM jobs WHERE id = $1 AND status = 'running' AND lease_token = $2",
        )
        .bind(job_id.0)
        .bind(lease_token)
        .fetch_optional(&self.pool)
        ...
        let result = sqlx::query(
            "UPDATE jobs SET status = $1, claimed_by = NULL, lease_token = NULL, lease_expires_at_ms = NULL, last_error = $2, next_attempt_at_ms = $3, updated_at_ms = $4 WHERE id = $5 AND lease_token = $6",
        )
        .bind(status)
        .bind(error)
    ...
    async fn depth(&self) -> Result<u64, JobQueueError> {
        let count: i64 = sqlx::query_scalar(
            "SELECT COUNT(*)::bigint FROM jobs WHERE status IN ('pending', 'running')",
        )
        .fetch_one(&self.pool)
        .await
        .map_err(|e| JobQueueError::Persistence(e.to_string()))?;
    ...
    async fn recover_expired_leases(&self, now: Timestamp) -> Result<u64, JobQueueError> {
        let result = sqlx::query(
            "UPDATE jobs SET status = 'pending', claimed_by = NULL, lease_token = NULL, lease_expires_at_ms = NULL, updated_at_ms = $1 WHERE status = 'running' AND lease_expires_at_ms <= $1",
        )
