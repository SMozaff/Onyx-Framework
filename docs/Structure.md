...
## Increment 7 additions

```text
crates/applications/
├── security-application/
│   └── src/ports/{authority_verifier,rate_limiter,secret_provider}.rs
└── audit-application/
    └── src/ports/{audit_writer,integrity}.rs
crates/infrastructure/
├── observability-adapter/
│   └── src/{tracing,metrics,logging,audit}.rs
├── security-adapter/
│   └── src/{authority,rate_limiter,secret_provider}.rs
└── background-jobs/
    └── src/{postgres_queue,sqlite_queue}.rs
crates/bins/worker/src/
├── job_runner.rs
├── scheduler_loop.rs
└── snapshot_loop.rs
crates/bins/api-server/src/middleware/rate_limit.rs
migrations/{postgres,sqlite}/
├── 20260103000000_add_job_queue.{up,down}.sql
└── 20260104000000_add_rate_limit.{up,down}.sql
crates/team7-integration-tests/
tests/integration/*_tests.rs
```

## Mobile v1.1 implementation additions
...
│   ├── background/
│   │   ├── sync_service.dart
│   │   ├── android/workmanager_service.dart
│   │   └── ios/background_service.dart
│   └── ui/
│       ├── app.dart
│       ├── screens/
│       │   ├── dashboard.dart
...
│   ├── java/com/onyx/MainActivity.kt
│   └── kotlin/com/onyx/WorkManagerService.kt
├── ios/Runner/
│   ├── AppDelegate.swift
│   ├── BackgroundService.swift
│   └── Info.plist
...
└── tool/
    ├── ensure_platform_scaffold.sh
    ├── build_rust_android.sh
    └── build_rust_ios.sh
```

Rust bridge additions:

```text
crates/mobile-core/src/ffi_mobile.rs
crates/applications/client-composition/src/sync_agent.rs  # conflict retention/resolution
```
